...
//      }
    const Comment = ({comData,condition})=>{
       if(comData===undefined) return;  
        ...
        <img className="w-7 ml-2 dark:w-auto dark:h-auto" src={isDarkMode ? WhitesUser : blackUser}></img>
        <h2 className='px-2 py-3 ml-2 text-lg '>{comData.name}</h2>
        </div>
        <p className='ml-8 px-3 -mt-2'>{comData.comments}</p>
        ...
        <img className="w-3 mt-2 " src={Down_Arrow}/> <p className='px-1'>replies</p>
    </button>)}
  ...
  return (
    <div className=' h-96 mt-5'>
            <h1 className='px-24 py-5 text-2xl font-bold' >Comments</h1>
